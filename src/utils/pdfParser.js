...
window.parsearBancoBCI = function(texto, mesAnio) {
    ...
    // Patrón para BCI: puede empezar con LUGAR, luego fecha DD/MM/YY, código, descripción, lugar, montos
    // Ejemplo: SANTIAGO 17/09/25 2209 14634522 PAYU *UBER EATS SANTIAGO $33.944 $33.944 01/01 $33.944
    const patron = /(?:[A-Z\s]+\s+)?(\d{2}\/\d{2}\/\d{2,4})\s+\d+\s+\d+\s+(.+?)\s+(?:[A-Z\s]+\s+)?\$\s*([\d\.]+)\s*\$\s*[\d\.]+\s+\d{2}\/\d{2}\s+\$/gi;
    ...
    console.log(`Parser BCI encontró ${transacciones.length} transacciones`);
...
window.parsearBancoChile = function(texto, mesAnio) {
    ...
    // Patrón para Edwards: puede empezar con LUGAR (opcional), luego fecha DD/MM/YY, código, descripción, montos
    // Ejemplo: SANTIAGO 20/08/25 210834426118 PAYU *UBER TRIP SANTIAGO $ 7.102 $ 7.102 01/01 $ 7.102
    // O también: 31/08/25 010900000000 Pago Pesos TEF $ -3.149.360 $ -3.149.360 01/01 $ -3.149.360
    const patron = /(?:[A-Z\s]+\s+)?(\d{2}\/\d{2}\/\d{2,4})\s+\d+\s+(.+?)\s+\$\s*([\-\d\.]+)\s*\$\s*[\-\d\.]+\s+\d{2}\/\d{2}\s+\$/gi;
    ...
    for (const linea of lineas) {
        ...
        for (const match of matches) {
            ...
            // Limpiar descripción: puede tener ubicación al final (SANTIAGO, LAS CONDES, etc)
            // Remover palabras de ubicación comunes al final
            ...
            // y que tengan una descripción válida
            if (monto > 0 && descripcion.length > 3 && !descripcion.match(/^[\d\s]+$/)) {
                transacciones.push({
                    fecha: fecha,
                    descripcion: descripcion,
    ...
    console.log(`Parser Edwards encontró ${transacciones.length} transacciones`);
